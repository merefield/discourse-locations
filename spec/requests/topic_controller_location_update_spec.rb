...
RSpec.describe TopicsController do
  ...
  describe "#update" do
    ...
    it "keeps a textual location and removes its previous spatial projection" do
      ...
      put "/t/#{topic.id}.json", params: { location: { raw: "Somewhere" } }

      expect(response.status).to eq(200)
      expect(topic.reload.custom_fields["location"]).to eq("raw" => "Somewhere")
