  ...
  class GeoNamesClient
    ...
    def initialize
      ...
      ::Locations::LoggingHelper.ip_lookup_log(
        "5. Locations GeoNamesClient initialized"
      )
      ::Locations::LoggingHelper.ip_lookup_log(
        "5. Locations GeoNamesClient base_url=#{@base_url}"
      )
    end
    ...
    def fetch_and_normalize(id)
      ::Locations::LoggingHelper.ip_lookup_log(
        "5. Locations GeoNames fetch: geoname_id=#{id}"
      )
      url =
        "#{@base_url}getJSON?" +
          URI.encode_www_form(
      ...
      ::Locations::LoggingHelper.ip_lookup_log(
        "5. Locations GeoNames request: url=#{url}"
      )
      ...
      ::Locations::LoggingHelper.ip_lookup_log(
        "5. GeoNames lookup failed (#{id}): #{e.message}"
      )
      nil
    end
  end
